...
t1=0.25;t2=0.8;s1=0.568;s2=0;  %位置
d1=2.5;d2=1.75;w1=0.8;w2=0;   %幅度
h=0.027;J=10;n=1024; %窗宽取0.027，分辨率取7（取大一点好），分辨率越小，变幅估计差得越远
FLAG=100;%迭代次数

filename = '\group12\Code-for-Geopolitics-and-Bitcoin\CorrelationAnalysis2\p2q1\BTC_USD Bitfinex.csv';
A = csvread(filename);
Y = A(:,1);
Y = Y(1:1024);
Y = Y';
...
volatilityfisrt = 0; volatilitypos1 = 0; Vop1 = 0; 
for i=1:6
...
fprintf('方差变点为：（位置，大小）');
VolatilityChange = [volatilitypos1, volatilityfisrt]
VolatilityPos(1) = VolatilityChange(1);
VolatilitySize(1) = VolatilityChange(2);
...
for flag = 2:FLAG
    ...
    CMsub1 = kde0(subsample1(1,:),0,VolatilityChange(1));
    estDensity1sub1 = kde1(subsample1(1,:), subsample1(2,:),0,VolatilityChange(1));
    coefficientsub1 = Coff(estDensity1sub1(2,:),0,VolatilityChange(1));  
    CVsub1 = CVE(subsample1(1,:),subsample1(2,:),0,VolatilityChange(1));
    statisticsub1 = Transform(coefficientsub1(2,:), CMsub1, CVsub1,0,VolatilityChange(1));  
    MeanChangeSub1 = ExtremePoint(statisticsub1(2,:),0,VolatilityChange(1));
    MeanChangeSub1(2,1) = ReturnCoff(MeanChangeSub1(:,1), CMsub1, CVsub1 );
    MeanChangeSub1(2,1) = ReturnValue(MeanChangeSub1(2,1),subsample1,0,VolatilityChange(1)); 
    MeanChangeSub1(2,2) = ReturnCoff(MeanChangeSub1(:,2), CMsub1, CVsub1 );
    MeanChangeSub1(2,2) = ReturnValue(MeanChangeSub1(2,2),subsample1,0,VolatilityChange(1)); 
    
    CMsub2 = kde0(subsample2(1,:),VolatilityChange(1),1);
    estDensity1sub2 = kde1(subsample2(1,:), subsample2(2,:),VolatilityChange(1),1);
    coefficientsub2 = Coff(estDensity1sub2(2,:),VolatilityChange(1),1); 
    CVsub2 = CVE(subsample2(1,:),subsample2(2,:),VolatilityChange(1),1);
    statisticsub2 = Transform(coefficientsub2(2,:), CMsub2, CVsub2 ,VolatilityChange(1),1); 
    MeanChangeSub2 = ExtremePoint(statisticsub2(2,:),VolatilityChange(1),1);
    MeanChangeSub2(2,1) = ReturnCoff(MeanChangeSub2(:,1), CMsub2, CVsub2 );
    MeanChangeSub2(2,1) = ReturnValue(MeanChangeSub2(2,1),subsample2,VolatilityChange(1),1); 
    MeanChangeSub2(2,2) = ReturnCoff(MeanChangeSub2(:,2), CMsub2, CVsub2 );
    MeanChangeSub2(2,2) = ReturnValue(MeanChangeSub2(2,2),subsample2,VolatilityChange(1),1); 
        
    MeanChangeRaw = [MeanChangeSub1, MeanChangeSub2]  %第一行位置，第二行大小
    ...
    fprintf('方差变点为：（位置，大小）');
    VolatilityChange = [volatilitypos1, volatilityfisrt]
    VolatilityPos(flag) = VolatilityChange(1);
    VolatilitySize(flag) = VolatilityChange(2);
...
%subplot(2,2,2)
i=1:1:FLAG;
plot(i, VolatilityPos(i), 'c', 'LineWidth', 1.5);
hold off;
xlabel('迭代次数');ylabel('位置估计');
title('方差变点位置估计（优先遗传）');
legend('VolatilityPos');
...
%subplot(2,2,4)
i=1:1:FLAG;
plot(i, VolatilitySize(i), 'c', 'LineWidth', 1.5);
hold off;
xlabel('迭代次数');ylabel('幅度估计');
title('方差变点幅度估计');
legend('VolatilitySize');
...
function res = estT2(sampleX,sampleY)
    ...
    den = sum(    exp( -(x-X).^2./(2*b^2) ) , 1 ) / (sqrt(2*pi)*b*N);
    res = num.^2./den;    %\hat{T^2}(Xi)的估计量
end
%返回值为统计量绝对值的最大值点和对应的统计量值（不含绝对值）
function res = ExtremePoint(Series,a,b)  %求非可微函数绝对值的极值点，思路：离散数列的极值点问题。返回两个极大值(一个的话写成double形式)
    N = length(Series);
    L = b-a;
    i = 1;
...
%返回值为统计量绝对值的最大值点和对应的统计量值（不含绝对值）
function res = ExtremePoint1(Series,a,b)
    N = length(Series);
    L = b-a;
    i = 1;
